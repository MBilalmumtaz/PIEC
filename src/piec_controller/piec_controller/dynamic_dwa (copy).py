...
class DynamicDWAPlanner:
    def __init__(self, node: Node, emergency_distance=0.3):
        ...
        # Sampling parameters
        self.v_samples = 12     # Increased for better coverage
        self.w_samples = 24     # Increased for better coverage
        
        # Prediction horizon
        self.dt = 0.1           # Smaller dt for smoother simulation
        self.sim_time = 2.0     # Longer simulation for better prediction
        
        # Cost weights - Balanced for safety and performance
        self.w_clearance = 2.5      # Increased safety emphasis
        self.w_heading = 1.5
        self.w_progress = 1.8
        self.w_speed = 1.0
        self.w_smoothness = 0.5
        self.w_obstacle_free = 3.0  # New: obstacle-free path weight
        self.w_path_alignment = 1.2  # New: alignment with global path
        ...
        self.dynamic_obstacle_threshold = 0.2  # m/s to consider as dynamic
        ...
        self.planning_times = []
        self.max_planning_time = 0.05  # 50ms max
    ...
    def set_pinn_service(self, pinn_service):
        """Set PINN service for energy/stability predictions"""
        self.pinn_service = pinn_service
        self.use_pinn = pinn_service is not None
        if self.use_pinn:
            self.node.get_logger().info("PINN service integrated into DWA planner")
    ...
    def plan_with_obstacle_awareness(self, pose, path, scan_ranges, scan_angles):
        ...
        self.last_best_v = best_cmd[0]
        self.last_best_w = best_cmd[1]
    ...
    def simulate_trajectory(self, pose, v, w):
        """Simulate trajectory with improved accuracy"""
        x, y, yaw = pose
        traj = []
        ...
        for t in np.arange(0, self.sim_time, self.dt):
            if abs(v) < 0.001:
                ...
                if abs(w) < 0.001:
                    ...
                    # Circular motion
                    radius = v / w
                    x += radius * (math.sin(yaw + w * self.dt) - math.sin(yaw))
                    y += -radius * (math.cos(yaw + w * self.dt) - math.cos(yaw))
                    yaw += w * self.dt
    ...
    def evaluate_trajectory_safety_advanced(self, traj, ranges, angles, robot_pose, dynamic_obstacles):
        """Advanced safety evaluation with dynamic obstacle consideration"""
        ...
        for i, (x, y, _) in enumerate(traj):
            # Convert to robot frame for each point
            dx = x - rx
            dy = y - ry
            point_dist = math.hypot(dx, dy)
            point_angle = math.atan2(dy, dx) - ryaw
            
            # Normalize angle
            point_angle = math.atan2(math.sin(point_angle), math.cos(point_angle))
            
            # Find closest laser reading to this point
            closest_distance = float('inf')
            for j, (range_val, angle) in enumerate(zip(ranges, angles)):
                if 0.1 < range_val < self.obstacle_influence_radius:
                    # Check if this laser beam intersects the trajectory point
                    angle_diff = abs(angle - point_angle)
                    if angle_diff < math.radians(30):  # Wider check cone
                        # Calculate minimum distance from point to laser beam
                        point_to_robot = math.hypot(dx, dy)
                        point_to_beam = abs(point_to_robot * math.sin(angle_diff))
                        
                        if point_to_beam < self.robot_radius:
                            distance_along_beam = point_to_robot * math.cos(angle_diff)
                            if 0 < distance_along_beam < range_val:
                                effective_distance = range_val - distance_along_beam
                                if effective_distance < closest_distance:
                                    closest_distance = effective_distance
            
            if closest_distance < float('inf'):
                closest_obstacle_dist = min(closest_obstacle_dist, closest_distance)
                
                # Calculate safety score
                if closest_distance < self.critical_distance:
                    safety = 0.0
                elif closest_distance < self.safe_distance:
                    safety = (closest_distance - self.critical_distance) / (self.safe_distance - self.critical_distance)
                else:
                    safety = 1.0
        ...
        for obstacle in dynamic_obstacles:
            for x, y, _ in traj:
                dist_to_obstacle = math.hypot(x - obstacle['x'], y - obstacle['y'])
                if dist_to_obstacle < self.safe_distance:
                    safety_reduction = 1.0 - (dist_to_obstacle / self.safe_distance)
                    min_safety = min(min_safety, 1.0 - safety_reduction)
                    closest_obstacle_dist = min(closest_obstacle_dist, dist_to_obstacle)
        
        return min_safety, closest_obstacle_dist
    def check_emergency_stop(self, ranges, angles):
        """Check for obstacles in immediate danger zone - FIXED"""
        if ranges is None or angles is None:
           return False
    
        emergency_zone = self.robot_radius + 0.15  # Reduced margin
    
        for i, (range_val, angle) in enumerate(zip(ranges, angles)):
            if abs(angle) < math.radians(45):  # Narrower front cone
               if 0.05 < range_val < emergency_zone:
                   self.node.get_logger().warn(f"Emergency: obstacle at {range_val:.2f}m, angle {math.degrees(angle):.1f}°")
                   return True
        return False  
    ...
    def generate_obstacle_aware_samples(self, ranges, angles, pose, dynamic_obstacles):
        ...
        if obstacle_density > 0.7:
            ...
            # v_samples = np.linspace(self.min_v, self.max_v * 0.4, …)   # OLD
            v_samples = np.linspace(self.min_v, self.max_v, self.v_samples)  # NO SCALE
