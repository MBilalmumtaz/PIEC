...
def generate_launch_description():
    ...
    # World selection argument - ONLY TWO WORLDS
    ...
    def launch_world(context, *args, **kwargs):
        ...
        # Map world names to their package and launch file - ONLY TWO WORLDS
        world_configs = {
            "aws_warehouse": ("aws_robomaker_small_warehouse_world", None),  # Special case
            "agriculture": ("agriculture_gazebo", "agriculture_world.launch.py"),
        }
        ...
        if world_name in world_configs:
            ...
            if world_name == "aws_warehouse":
                ...
                # Standard world launch for agriculture
    ...
    # UNIVERSAL CMD_VEL BRIDGE – forwards /cmd_vel to all possible Gazebo topics
    cmd_vel_bridge_universal = Node(
        package="ros_gz_bridge",
        executable="parameter_bridge",
        name="cmd_vel_bridge_universal",
        arguments=[
            # ROS -> GZ: map /cmd_vel to multiple Gazebo topics
            "/cmd_vel@geometry_msgs/msg/Twist]ignition.msgs.Twist",
            "/cmd_vel@geometry_msgs/msg/Twist]/scout/cmd_vel",
            "/cmd_vel@geometry_msgs/msg/Twist]/model/scout_mini/cmd_vel",
        ],
        parameters=[{"use_sim_time": True}],
        output="screen",
    )

    # Joint state bridge for Gazebo Sim - FIXES THE WHEEL FLUCTUATION
    joint_state_bridge = Node(
        package="ros_gz_bridge",
        executable="parameter_bridge",
        name="joint_state_bridge",
        arguments=[
            "/model/scout_mini/joint_state@sensor_msgs/msg/JointState[ignition.msgs.Model"
        ],
        parameters=[{"use_sim_time": True}],
        output="screen",
    )
